...
profile = False
THEANO_FLAGS='device=cuda,floatX = float32,nvcc.flags=-D_FORCE_INLINES'
#-----------------------------------------------------------------------------#
# Specify model and table locations here
# From https://github.com/ryankiros/skip-thoughts
#-----------------------------------------------------------------------------#
path_to_models = '/home/ms6771/MLProject/Skipthought/'
path_to_tables = '/home/ms6771/MLProject/Skipthought/'
#-----------------------------------------------------------------------------#
...
df_eng = pd.read_csv('data.csv', sep = '\t', dtype = {'text': str})
df_eng = df_eng[['text','stars']]
df_eng = shuffle(df_eng)
# df_eng = df_eng[:50000]

# Remove non-ascii characters and Downsample so that each star has the same number of reviews
...
df_eng = df_eng.dropna(axis=0, how='any')
df_eng['text'] = df_eng['text'].astype(str)
df_eng['stars'] = df_eng['stars'].astype(int)
data_list_, label_list_ = balance_classes(df_eng['text'].tolist(), df_eng['stars'].tolist())

data_list = [''.join([st for st in s if st in string.printable]) for s in data_list_]
label_list = [s for s in label_list_]

print("length before: {}".format(df_eng.shape[0]))
print("length after: {}".format(len(data_list)))

# Encode reviews to Skipthought vectors
data_label = []

for i in range(len(data_list)):
    start = time.time()
    sentences = tokenize.sent_tokenize(data_list[i])
    sent_vectors = encoder.encode(sentences)
    data_label.append((sent_vectors, label_list[i]))
    end = time.time()
    print("Encode progress: {}/{} | Takes: {} seconds".format(i, len(data_list), (end - start)))

# Save encoding result
torch.save(data_label, 'datalabel50000')
print("data label list saved")
